...
export function ApplicationsProvider({ children }: Readonly<{ children: ReactNode }>) {
  ...
  const addApplication = async (listingId: number, _userEmail: string) => {
    ...
    // Reload the applications so the candidate information
    // comes directly from the backend.
    const refreshResponse = await fetch(
      'http://localhost:8080/candidacy',
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    if (refreshResponse.ok) {
      const updatedApplications =
        await refreshResponse.json() as Application[]
  ...
  const updateApplicationStatus = async (
    ...
    const response = await fetch(
      'http://localhost:8080/candidacy/update',
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
  ...
  const removeApplication = async (id: number) => {
    ...
    const response = await fetch(
      `http://localhost:8080/candidacy/delete?id=${id}`,
      {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
